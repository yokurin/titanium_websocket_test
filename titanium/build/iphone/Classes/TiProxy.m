...
//Rare exceptions to indicate a bug in the _tisocket_io code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_tisocket_io = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_tisocket_io]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.tisocket_io.module"
